...
generatorRouter.post('/video/generate', async (req, res) => {
  try {
    const { prompt, sourceImageUrl, duration = 6, aspectRatio = '9:16', segments, totalDuration, voiceoverScript, voiceoverVoice, musicStyle, enableCaptions } = req.body as {
      prompt: string;
    ...
    await videoGenerateQueue.add(
      `video-${jobId}`,
      { prompt, sourceImageUrl, duration, aspectRatio, userId, jobId, segments, totalDuration, voiceoverScript, voiceoverVoice, musicStyle, enableCaptions },
      {
        attempts: 1,
